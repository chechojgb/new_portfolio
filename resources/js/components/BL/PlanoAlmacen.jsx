...
const PlanoAlmacen = ({ onEstanteriaClick, productos }) => {
    // ... (mantener las funciones productosPorEstanteria y getNombreEstanteria igual)
    ...
    const [zonaActiva, setZonaActiva] = useState('izquierda');
    ...
    return (
        <div className="flex-1 bg-white/80 dark:bg-gray-900 backdrop-blur-sm border border-white/30 dark:border-gray-700 shadow-2xl rounded-3xl p-6 overflow-hidden relative transition-colors duration-300">
            ...
            {/* NAVEGACIÓN POR PESTAÑAS (siempre visible) */}
            <div className="flex mb-4 border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden">
                <button 
                    ...
                    onClick={() => setZonaActiva('izquierda')}
                >
                     Zona Izquierda
                </button>
                <button 
            ...
            <div className="relative w-full h-[calc(100%-80px)] bg-gray-50 dark:bg-gray-800 rounded-xl border-2 border-dashed border-gray-300">
                ...
                {/* ZONA IZQUIERDA COMPLETA */}
                <div className={`${zonaActiva === 'izquierda' ? 'block' : 'hidden'} absolute top-0 left-0 w-full h-full`}>
                    
                    {/* PARTE SUPERIOR IZQUIERDA (20%) */}
                    <div className="absolute top-0 left-0 w-full h-1/5 flex p-4 gap-4">
                        ...
                        {/* MITAD DERECHA SUPERIOR: INDICADOR DE ZONA DERECHA */}
                        <div 
                            className="w-1/2 h-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700 border-2 border-dashed border-gray-400 dark:border-gray-600 rounded-xl flex flex-col justify-center items-center cursor-pointer hover:scale-105 transition-all duration-200"
                            onClick={() => setZonaActiva('derecha')}
                        >
                            <div className="text-center text-gray-500 dark:text-gray-300">
                                <div className="text-2xl mb-1"></div>
                                <p className="text-xs">ZONA DERECHA</p>
                                <p className="text-xs mt-1">(Click para cambiar)</p>
                            </div>
                        </div>
                    </div>

                    {/* PARTE INFERIOR IZQUIERDA (80%) */}
                ...
                {/* ZONA DERECHA COMPLETA */}
                <div className={`${zonaActiva === 'derecha' ? 'block' : 'hidden'} absolute top-0 left-0 w-full h-full`}>
                    
                    {/* PARTE SUPERIOR DERECHA (20%) */}
                    <div className="absolute top-0 left-0 w-full h-1/5 flex p-4 gap-4">
                        {/* MITAD IZQUIERDA SUPERIOR: INDICADOR DE ZONA IZQUIERDA */}
                        <div 
                            className="w-1/2 h-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700 border-2 border-dashed border-gray-400 dark:border-gray-600 rounded-xl flex flex-col justify-center items-center cursor-pointer hover:scale-105 transition-all duration-200"
                            onClick={() => setZonaActiva('izquierda')}
                        >
                            <div className="text-center text-gray-500 dark:text-gray-300">
                                <div className="text-2xl mb-1"></div>
                                <p className="text-xs">ZONA IZQUIERDA</p>
                                <p className="text-xs mt-1">(Click para cambiar)</p>
                            </div>
                        </div>

                        {/* MITAD DERECHA SUPERIOR: ZONA DE EMPAQUETACIÓN */}
